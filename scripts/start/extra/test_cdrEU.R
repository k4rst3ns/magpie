...
.title <- function(version = NULL, miti = NULL, agf = NULL, scm = NULL, regions = NULL, cdr = NULL){
  return(paste(version, miti, agf, scm, regions, cdr, sep = "_"))
}
  ...
  for (regions in regionSet){
    for (cdrReg in c("eu", "glo")){     
      ...
      if(regions == "h12") {
        ...
        cfg$input['cellular']    <- "rev4.118_36f73207_b62ca1de_cellularmagpie_c200_MRI-ESM2-0-ssp126_lpjml-8e6c5eb1_clusterweight-ba4466a8.tgz"
      ...
      for(agf in names(agfScen)) {
        for(scm in names(scmScen)) {
          ...
          cfg$title <- .title(version, scen, agf, scm, regions, cdrReg)
