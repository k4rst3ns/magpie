...
q21_trade_lower_bloc(i_ex,i_im,k_trade)..
 v21_trade(i_ex,i_im,k_trade) =g=
    ...
    * sum(ct, i21_import_supply_historical(i_ex,i_im,ct,k_trade) * i21_import_supply_scenario(ct)
       - i21_stddev_lib_factor(ct) * i21_trade_bilat_stddev(ct,i_ex,i_im,k_trade)
         * (1 + (s21_intrabloc_lib_factor - 1) * i21_bloc_trade_indicator(i_ex,i_im)))
    * (i21_bloc_trade_indicator(i_ex,i_im) + (1 - i21_bloc_trade_indicator(i_ex,i_im)) * s21_crossbloc_lower_bound_factor);


*' Modified lower bound for trade with bloc-specific behavior:
*' - Intra-bloc: additional liberalization via s21_intrabloc_lib_factor (wider cone)
*' - Cross-bloc: lower bound multiplied by s21_crossbloc_lower_bound_factor
*'   When factor = 0, model can endogenously reduce cross-bloc trade to zero


*' Modified upper bound:
*' - Intra-bloc: additional liberalization via s21_intrabloc_lib_factor
*' - Cross-bloc: normal upper bound (no cap - trade reduction is endogenous via lower bound)

q21_trade_upper_bloc(i_ex,i_im,k_trade)..
...
q21_costs_tariffs_bloc(i2,k_trade)..
 v21_cost_tariff_reg(i2,k_trade) =e= sum((ct,i_im), v21_trade(i2,i_im,k_trade) * i21_bloc_tariff(ct,i2,i_im,k_trade));
